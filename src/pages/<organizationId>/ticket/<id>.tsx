...
export function ChatBubble({ chat }: { chat: Chat }) {
    return (
        <div
            class={`flex ${chat.sender === "employee" ? "justify-start" : "justify-end"
                } my-2`}
        >
            <div
                class={`rounded-lg px-4 py-2 text-lg shadow ${chat.sender === "costomer" ? "bg-gray-200" : "bg-gray-700 text-white"
                    }`}
            >
                <span safe>{chat.message}</span>
                <div
                    class={`mt-2 text-xs ${chat.sender === "costomer" ? "text-gray-600" : "text-gray-300"
                        }`}
                >
